...
// Validate inputs
if ($classroom_id <= 0 || $user_id <= 0 || empty($start_date) || empty($end_date) || empty($start_time) || empty($end_time)) {
    http_response_code(400);
    echo json_encode([
        "success" => false,
...
// Insert booking
$query = "INSERT INTO classroom_bookings (classroom_id, start_date, end_date, start_time, end_time, user_id, created_at, updated_at) 
          VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())";
$stmt = $conn->prepare($query);
$stmt->bind_param("issssi", $classroom_id, $start_date, $end_date, $start_time, $end_time, $user_id);
